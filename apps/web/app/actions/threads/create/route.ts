...
export async function POST(request: NextRequest) {
  ...
  let anchor: unknown;
  try {
    anchor = JSON.parse(anchorJson) as unknown;
  } catch {
    return redirectWithFlash(
      request,
      returnTo,
      "error",
      "NotebookLens could not parse the selected thread anchor.",
    );
  }
  ...
  return redirectWithFlash(request, returnTo, "success", "Thread created.");
}
